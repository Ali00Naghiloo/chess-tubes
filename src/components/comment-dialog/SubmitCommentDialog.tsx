...
type FormValuesProps = {
  ...
  title?: string;
  commentText: string;
  positiveIdea: ProsOrConsItem[];
  negativeIdea: ProsOrConsItem[];
  isAnonymous: boolean;
  quality?: string;
  buyingWorth?: string;
};
...
function SubmitCommentDialogForm({ handleClose, type, name }: SubmitCommentDialogFormProps) {
  ...
  const SubmitCommentSchema = Yup.object().shape({
    ...
    positiveIdea: Yup.array().required(''),
    negativeIdea: Yup.array().required(''),
    isAnonymous: Yup.boolean().required(''),
    quality: Yup.string().optional(),
    buyingWorth: Yup.string().optional(),
  });
  ...
  const onSubmit = (data: FormValuesProps) => {
    try {
      if (router.isReady) {
        ...
        const commentData = {
          ...data,
          positiveIdea: JSON.stringify(data.positiveIdea),
          negativeIdea: JSON.stringify(data.negativeIdea),
          isAnonymous: Number(data.isAnonymous),
        };
        dispatch(
          (type === 'course' ? sendCourseComment : sendProductComment)(
  ...
  const negativeIdea = watch('negativeIdea');

  const positiveIdea = watch('positiveIdea');

  const quality = watch('quality');

  const buyingWorth = watch('buyingWorth');
    ...
    >
      <Paper variant="outlined" sx={{ p: 2, px: 3 }}>
        ...
        <Stack spacing={{ sm: 4, xs: 8 }}>
          ...
          <ValueResponseChips setValue={setValue} buyingWorth={buyingWorth} />
        </Stack>
        <Divider sx={{ my: 3 }} />
...
const CHIPS_MAX_LENGTH = 5;

type CommentProsProps = {
  setValue: UseFormSetValue<FormValuesProps>;
  chips: ProsOrConsItem[];
  inputLabel: string;
  chipColor: 'success' | 'error';
  type: 'negativeIdea' | 'positiveIdea';
};

function CommentProsOrCons({ chips, type, chipColor, inputLabel, setValue }: CommentProsProps) {
  ...
  const addToChips = () => {
    if (inputValue.trim() === '' || chips.length === CHIPS_MAX_LENGTH) {
      return;
    }
    setValue(type, [...chips, { id: uuidv4(), value: inputValue }]);
    setInputValue('');
  };
  ...
  return (
    <>
      <TextField
        ...
        value={inputValue}
        onChange={handleChange}
        helperText="حداکثر ۵ نکته"
        InputProps={{
          endAdornment: (
            <InputAdornment position="end">
              <IconButton onClick={addToChips} edge="end">
