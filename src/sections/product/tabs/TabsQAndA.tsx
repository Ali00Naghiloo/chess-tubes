...
// modules
import getProductQAndA from '@/modules/q&a/redux/operators/getProductQAndA';
...
export default function TabsQAndA() {
  ...
  useEffect(() => {
    if (router.isReady) {
      dispatch(getProductQAndA(`${params.product_id}`, id, successCallback));
    }
  ...
  return (
    <>
            ...
            >
              ...
              <Box flexGrow={1}>
                ...
                <Box sx={{ overflow: 'hidden', height: 500 }}>
                  ...
                  >
                    <Stack divider={<Divider sx={{ my: 1 }} />} spacing={2}>
      ...
      <SubmitQuestionDialog name={title} open={isDialogOpen} handleClose={handleDialogClose} />
    </>
  );
}
...
function QAndA({ answers, id, question }: QAndAProps) {
  //
  const [isDialogOpen, setIsDialogOpen] = useState(false);
