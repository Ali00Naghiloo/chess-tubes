...
// operators
import getExternalNews from '@/modules/news/redux/operators/getExternalNews';
...
export default function NewsNew() {
  ...
  const { data, current_page, last_page } = useSelector((s) => s.news.externalNews);
  ...
  useEffect(() => {
    ...
    searchParams.append('type', 'international');

    setIsLoading(true);
    dispatch(getExternalNews(searchParams.toString(), successCallback, failureCallback));
  }, [filter, page]);
