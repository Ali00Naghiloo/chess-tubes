...
import getOnlineTrainingAndA from '@/modules/q&a/redux/operators/getOnlineTrainingQAndA';
//
...
export default function TabsQAndA() {
  ...
  const { id: idd, title, isCurrentUserBuyer } = useSelector((s) => s.onlineCourse.course);
  ...
  useEffect(() => {
    if (router.isReady) {
      dispatch(getOnlineTrainingAndA(idd, id, successCallback));
    }
...
function AnswerItem({ answer, dislike, id, like, name }: AnswerItemProps) {
  ...
  const { id: idd } = useSelector((s) => s.onlineCourse.course);
  ...
  const likeOrDislike = (t: 'like' | 'dislike') => {
    if (router.isReady) {
      setIsLoading(true);
      setType(t);
      dispatch(likeOrDislikeAnswer(idd, userId, id, t, 'course', successCallback, failureCallback));
    }
  };
