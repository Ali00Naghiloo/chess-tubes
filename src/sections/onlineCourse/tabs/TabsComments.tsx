...
import getOnlineCourseComments from '@/modules/comment/redux/operators/getOnlineCourseComments';
import { SubmitCommentDialog } from '@/components/online-comment-dialog';
import { commentService } from '@/modules/comment/service';
import { successfulGetComment, successfulGetHaveComment } from '@/modules/comment/redux/actions';
...
export default function TabsComments() {
  ...
  const { id, title, isCurrentUserBuyer } = useSelector((s) => s.onlineCourse.course);
  ...
  useEffect(() => {
    if (router.isReady) {
      dispatch(getOnlineCourseComments(id, user?.id, successCallback));
    }
            ...
            <EmptyContent
              ...
              action={
                <Button
                  ...
                  disableElevation
                  onClick={() => {
                    commentService.getOnlineCourseComment(id).then((res) => {
                      dispatch(successfulGetHaveComment(res.data.data));
