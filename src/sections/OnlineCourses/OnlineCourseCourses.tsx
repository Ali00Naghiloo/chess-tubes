...
// operators
import getListOfCourses from '@/modules/online-course/redux/operators/getListOfCourses';
// components
import SortSelect from '@/components/select-sort/SelectSort';
import CourseCard from '@/components/online-course-card';
...
export default function CourseCourses() {
  ...
  const { data, current_page, last_page } = useSelector((s) => s.onlineCourse.courses);
          ...
          <Grid container spacing={{ xs: 1, sm: 2 }} justifyContent="flex-start">
            {data.map((p) => (
              <Grid key={p.id} item xs={12 / 2} sm={12 / 3} md={12 / 3} lg={12 / 4}>
                <CourseCard {...p} id={String(p.id)} sx={{ width: 'auto', mb: 2 }} />
              </Grid>
            ))}
          </Grid>
