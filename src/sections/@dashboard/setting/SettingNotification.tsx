...
export default function SettingNotification() {
  ...
  const [notificationType, setNotificationType] = useState<any>({
    ...
    sendsms: sendsms || 0,
    // sendnotife: notificationSetting.sendnotife,
  });
  ...
  return (
    <Paper variant="outlined" sx={{ py: 2, px: { sm: 2, md: 3, lg: 4, xs: 2 } }}>
      ...
      <Stack sx={{ px: 2 }} alignItems="flex-start">
        <FormControlLabel
          label="از طریق ایمیل"
          control={
            <Checkbox
        ...
        {/* <FormControlLabel
          label="از طریق اعلان های سایت"
          control={
            <Checkbox
              name="sendnotife"
              checked={Boolean(notificationType.sendnotife)}
              onChange={handleChange}
            />
          }
        /> */}
      ...
      <Stack sx={{ px: 2 }} direction="row" alignItems="flex-start" spacing={1}>
        <LoadingButton
